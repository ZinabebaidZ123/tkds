{{-- Path: resources/views/admin/about/timeline/index.blade.php --}}
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
            ...
            <h2 class="text-2xl font-bold text-gray-900">Manage Company Timeline</h2>
            <p class="text-gray-600 text-sm mt-1">Control the timeline milestones displayed in the About page</p>
        </div>
        <a href="{{ route('admin.about.timeline.create') }}" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $timeline->total() }}</p>
                </div>
                <i class="fas fa-history text-3xl text-blue-200"></i>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $timeline->where('status', 'active')->count() }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">Left Side</p>
                    <p class="text-2xl font-bold">{{ $timeline->where('position', 'left')->count() }}</p>
                </div>
                <i class="fas fa-arrow-left text-3xl text-purple-200"></i>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-100 text-sm font-medium">Right Side</p>
                    <p class="text-2xl font-bold">{{ $timeline->where('position', 'right')->count() }}</p>
                </div>
                <i class="fas fa-arrow-right text-3xl text-orange-200"></i>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                    ...
                    All Timeline Items ({{ $timeline->total() }})
                </h3>
            </div>
        </div>
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Timeline Item</th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200 group">
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-gray-500 max-w-xs line-clamp-2">{{ Str::limit($item->description, 80) }}</div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        <div class="w-3 h-3 {{ $item->getColorClass() }} rounded-full"></div>
                                        <span class="text-sm text-gray-600 capitalize">
                                            <i class="fas fa-arrow-{{ $item->position }} mr-1"></i>
                                            {{ $item->position }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 border border-gray-300">
                                ...
                                <td class="px-6 py-4">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        ...
                                        <span class="ml-3 text-sm font-medium status-text {{ $item->isActive() ? 'text-green-600' : 'text-red-600' }}">
                                            {{ ucfirst($item->status) }}
                                        </span>
                                    </label>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        <a href="{{ route('admin.about.timeline.edit', $item) }}" 
                ...
                @foreach($timeline as $item)
                    <div class="p-6 hover:bg-gray-50 transition-colors duration-200">
                        <div class="flex space-x-4">
                            <div class="flex-shrink-0">
                                <div class="w-12 h-12 {{ $item->getColorClass() }} rounded-lg flex items-center justify-center">
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center mt-2 space-x-4">
                                            ...
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $item->isActive() ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ ucfirst($item->status) }}
                                            </span>
                                        </div>
                                    </div>
        ...
        @else
            <div class="text-center py-16">
                ...
                <p class="text-gray-500 mb-6 max-w-sm mx-auto">Get started by creating your first timeline milestone to showcase your company's journey.</p>
                <a href="{{ route('admin.about.timeline.create') }}" 
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl transform transition-all duration-300 scale-95">
        <div class="text-center">
            ...
            <p class="text-gray-600 mb-6">Are you sure you want to delete this timeline item? This action cannot be undone and will remove it from the About page.</p>
...
document.querySelectorAll('.status-toggle').forEach(toggle => {
    toggle.addEventListener('change', function() {
        ...
        fetch(`{{ route('admin.about.timeline.index') }}/${id}/status`, {
            method: 'POST',
            headers: {
...
document.getElementById('confirmDelete').addEventListener('click', function() {
    if (deleteId) {
        ...
        form.method = 'POST';
        form.action = `{{ route('admin.about.timeline.index') }}/${deleteId}`;
        form.innerHTML = `
